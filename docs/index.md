...
on the business logic". A strong facilitator move: when a test goes green,
**deliberately break the production code** to show the test fails. This proves the
Nullable runs real code, not a stand-in — one participant did exactly this to
convince himself `createNull` was trustworthy ("it's too green for my taste, let's
break it").
