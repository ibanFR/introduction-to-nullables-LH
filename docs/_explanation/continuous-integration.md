...
build and test the Java implementation and to deploy this documentation site.
...
While currently configured for Java 17 only, the matrix structure allows for easy expansion to test against multiple
...
subdirectory where the [`pom.xml`]({{ site.repository }}/blob/main/java/pom.xml) file is located.
...
After the configuration is complete, the [`.github/workflows/pages.yml`]({{ site.repository }}/blob/main/.github/workflows/pages.yml) workflow will automatically build and deploy the
documentation site to GitHub Pages whenever changes under the [`docs/`]({{ site.repository }}/tree/main/docs) directory are pushed to the `main` branch.
